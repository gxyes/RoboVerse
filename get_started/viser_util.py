...
from metasim.cfg.objects import PrimitiveCubeCfg, PrimitiveSphereCfg, PrimitiveCylinderCfg, RigidObjCfg, ArticulationObjCfg
...
# ====== load urdf ======
def load_urdf_with_patch(urdf_path: str):
    import yourdfpy
...
class ViserVisualizer:
    ...
    def add_frame(self, name: str, show_axes=True):
    ...
    def visualize_item(self, cfg: Union[PrimitiveCubeCfg, PrimitiveSphereCfg, PrimitiveCylinderCfg, RigidObjCfg, ArticulationObjCfg, BaseRobotCfg], 
                        name: str, state: Optional[dict] = None, base_offset_debug: bool = True):
    ...
    def update_item_pose(self, name: str, state: dict):
        """
        Update URDF/primitive items.
        Support visualize_item rendered objects。
        """
        import numpy as np
        # 1. URDF/robots
    ...
    def load_trajectory(self, trajectory_path: str):
        ...
        Args:
            trajectory_path: Path to the trajectory file (.pkl, .pkl.gz, .json, .yaml)
        """
        try:
    ...
    def set_current_trajectory(self, robot_name: str, demo_index: int = 0):
        ...
        self._current_frame = 0
        logger.info(f"Set current trajectory: {robot_name}, demo {demo_index}, {len(self._current_trajectory['actions'])} frames")
        return True
    ...
    def create_joint_sliders(self, robot_name: str, client_gui):
        ...
        has_saved_positions = (robot_name in self._current_joint_positions and 
                             len(self._current_joint_positions[robot_name]) == len(joint_limits))
        ...
        for i, (joint_name, (lower, upper)) in enumerate(joint_limits.items()):
            ...
            def make_update_callback(robot_name, slider_list):
                def update_callback(_):
                    if robot_name in self._urdf_handles and self._urdf_handles[robot_name]:
    ...
    def enable_camera_controls(self, 
        ...
        def setup_client_camera(client):
            ...
            try:
                ...
                client.camera.position = np.array([initial_position[0]*2, initial_position[1]*2, initial_position[2]*2])  # Offset main view
                ...
                with client.gui.add_folder("Camera Controls"):
                    pos_x = client.gui.add_slider("Camera X", min=-10.0, max=10.0, step=0.2, initial_value=initial_position[0])
                    pos_y = client.gui.add_slider("Camera Y", min=-10.0, max=10.0, step=0.2, initial_value=initial_position[1])
                    pos_z = client.gui.add_slider("Camera Z", min=0.1, max=10.0, step=0.2, initial_value=initial_position[2])
                    ...
                    fov_slider = client.gui.add_slider("Camera FOV", min=20.0, max=90.0, step=1.0, initial_value=initial_fov)
                ...
                def update_camera_view():
                    ...
                    try:
                        ...
                        logger.info(f"Rendered image shape: {image.shape}, dtype: {image.dtype}, min: {image.min()}, max: {image.max()}")
                        
                        # Check if image is completely black or has content
                        non_zero_pixels = np.count_nonzero(image)
                        total_pixels = image.size
                        logger.info(f"Non-zero pixels: {non_zero_pixels}/{total_pixels} ({100*non_zero_pixels/total_pixels:.1f}%)")
                ...
                def reset_camera():
                    ...
                    current_camera_rotation = np.eye(3)
                    current_camera_rotation = orthogonalize_matrix(current_camera_rotation)  # Ensure perfect orthogonality
                    camera_frustum.wxyz = matrix_to_quaternion(current_camera_rotation)
    ...
    def enable_trajectory_playback(self):
        """Enable trajectory playback controls for all connected clients."""
        @self.server.on_client_connect
        def setup_trajectory_controls(client):
            ...
            try:
                ...
                with client.gui.add_folder("Trajectory Playback"):
                    trajectory_status = client.gui.add_text("Trajectory Status", initial_value="Use: visualizer.load_trajectory('path')")
                    ...
                    robot_select = client.gui.add_dropdown("Robot", options=["No trajectories loaded"], initial_value="No trajectories loaded")
    ...
    def enable_joint_control(self):
        """Enable joint control for all connected clients."""
        @self.server.on_client_connect
        def setup_joint_controls(client):
            ...
            try:
                ...
                with client.gui.add_folder("Joint Control"):
                    ...
                    robot_select_joint = client.gui.add_dropdown("Control Robot", options=["No robots loaded"], initial_value="No robots loaded")
                ...
                def clear_joint_control_gui():
                    ...
                    if cleared_robots:
                        joint_status.value = f"Cleared joint control panels for: {', '.join(cleared_robots)} (positions preserved)"
                        logger.info(f"Cleared GUI panels but preserved joint positions for: {', '.join(cleared_robots)}")
                    else:
                ...
                def reset_joints_gui():
                    ...
                    if selected_robot in self._joint_sliders:
                        ...
                        if selected_robot in self._urdf_handles and self._urdf_handles[selected_robot]:
                            try:
